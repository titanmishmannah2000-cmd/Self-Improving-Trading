...
- **Audit B9 (CLOSED):** "All indicators credited equally" — the old loop credited EVERY discovered indicator for a pair on every GP close (via `_discovered_indicator_ids`), so the Per-Indicator WR card was meaningless + always empty (no per-indicator PnL tracked). Fixed in `5d0d9ea`: `gp_ensemble_signal` now returns the firing indicator IDs in `meta["gp_indicators"]`; the loop carries them on the position at open; on a GP close it credits ONLY those indicators via `cortex.record_indicator_outcome(ind, pnl, entry_type="gp_ensemble")`. `record_indicator_outcome` now also tracks per-indicator `pnl` + a separate `gp` sub-block (GP-entry attempts/wins/pnl), and `summary()` surfaces real `pnl` + `by_type.gp_ensemble` (so the dashboard's Per-Indicator card shows real numbers + a GP-Entry WR column). VERIFIED by 31 targeted tests (incl new `test_gp_close_credits_only_firing_indicators`) + local proof that Z (non-firing) is never credited and per-indicator PnL/WR accumulate correctly. LIVE: bots deployed; Per-Indicator card populates on the next GP close (no GP close had occurred in the verification window yet).
