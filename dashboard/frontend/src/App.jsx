      ...
      <div className="pc-head">
        ...
        {!isWatcher && (
          <div className="pc-strategies">
            ...
            {data?.gpGate?.banned && (
              <span
                className="pc-strategy pc-strategy-gp-banned"
                data-testid="gp-banned-badge"
                title={data.gpGate.recommendation_reason || data.gpGate.last_reason || "GP Brain promote entries blocked"}
              >
                GP banned
              </span>
            )}
            {!data?.gpGate?.banned && data?.gpGate?.recommendation === "should_exclude" && (
              <span
                className="pc-strategy pc-strategy-gp-exclude-hint"
                data-testid="gp-exclude-hint"
                title={data.gpGate.recommendation_reason || "Expectancy suggests putting this pair on GP exclude"}
              >
                Exclude?
              </span>
            )}
            {data?.gpGate?.recommendation === "should_include" && (
              <span
                className="pc-strategy pc-strategy-gp-include-hint"
                data-testid="gp-include-hint"
                title={data.gpGate.recommendation_reason || "Expectancy suggests removing this pair from GP exclude"}
              >
  ...
  return (
    <>
      <div className={`detail-panel ${maximized ? "detail-panel-maximized" : ""}`}>
        ...
        {(() => {
          const gate = (botData?.gp_promote_gate || {})[pair];
          if (!gate || typeof gate !== "object") return null;
          const banned = !!gate.banned;
          const rec = gate.recommendation || "hold";
          const reason = gate.recommendation_reason || "";
          const lastReason = gate.last_reason || "—";
          const n = gate.n ?? 0;
          const exp = gate.expectancy;
          const expTxt = typeof exp === "number" ? `${exp.toFixed(4)}%` : "—";
          let calloutTitle = "No exclude change needed";
          let calloutClass = "gp-gate-callout-hold";
          if (rec === "should_exclude") {
            calloutTitle = "This pair should be on GP exclude";
            calloutClass = "gp-gate-callout-exclude";
          } else if (rec === "should_include") {
            calloutTitle = "This pair should be out of GP exclude";
            calloutClass = "gp-gate-callout-include";
          }
          return (
            <div className="detail-gp-gate" data-testid="detail-gp-gate">
              <div className="dc-label">GP Brain / exclude</div>
              <div className="dp-row">
                <span>Status</span>
                <span className="mono" data-testid="detail-gp-status">
                  {banned ? "Banned" : "Allowed"}
                </span>
              </div>
              <div className="dp-row">
                <span>Env listed</span>
                <span className="mono">
                  {gate.env_listed ? "yes (GP_EXCLUDE_PAIRS)" : "no"}
                  {gate.seeded_from_env ? " · seeded" : ""}
                </span>
              </div>
              <div className="dp-row">
                <span>Last gate reason</span>
                <span className="mono">{lastReason}</span>
              </div>
              <div className="dp-row">
                <span>Samples / expectancy</span>
                <span className="mono">n={n} · {expTxt}</span>
              </div>
              <div className={`gp-gate-callout ${calloutClass}`} data-testid="detail-gp-recommendation">
  ...
  for (const [botName, bot] of Object.entries(overview?.bots || {})) {
    ...
    for (const pair of Object.keys(PAIR_META)) {
      ...
      pairData[pair] = {
        ...
        gpGate: (bot.gp_promote_gate || {})[pair] || null,
      };
    }
  }
